...
**CLI Options:** `--output-dir`, `--config`, `--open`, `--verbose`, `--log-file`, `--provider`, `--api-key`, `--model`.
...
```
Does the function's actual decorator name exist in UNSAFE_DECORATOR_PATTERNS?
├── YES → status: "unsafe"
└── NO ↓
...
def get_llm(config: AnalysisConfig):
    ...
    if config.llm_provider == "anthropic":
        ...
        return ChatGroq(model=config.llm_model, temperature=0.1)
    # Default models and API keys are managed in AnalysisConfig
```
...
The `repolens/utils/logger.py` module provides unified logging:
- `get_logger(component)` for deterministic analysis modules.
- `log_node_start` and `log_node_end` for LangGraph node timing.
- `log_llm_call` for tracking prompts and tokens.
- `_session` dictionary tracks total calls, tokens, and duration at the session level.
- `log_session_summary()` prints the final CLI summary panel metrics.
...
The report is rendered using a fully rewritten HTML template with a dark theme, sidebar navigation, score circle, stat cards, and Mermaid dark theme. Section headers exist only inside the section renderers (e.g., `overview.py`), not in the global template.
...
## Executive Summary
Architecture Score: {score}/1.0  [{Critical / Poor / Fair / Good / Excellent (> 0.9)}]
...
└── tests/
    ...
    ├── unit/                        # 163 unit tests across all files
    │   ├── test_logger.py
    │   ├── test_ast_parser.py
